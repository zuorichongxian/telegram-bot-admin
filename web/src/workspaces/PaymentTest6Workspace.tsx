...
import {
  ...
  createUnifiedOrder,
  DEFAULT_PAYMENT6_CONFIG,
  fetchPayment6Callbacks,
  formatFenToYuan,
  generateOrderNo,
  getDefaultPayment6CallbackUrls,
  getPayment6ApiConfig,
  isProxyPayment6ApiConfig,
  isValidFenAmount,
  maskKey,
  PAYMENT6_STATE_MAP,
  queryBalance,
  queryOrder,
  type Payment6ApiResult,
  type Payment6Callback,
  type Payment6Config,
  type QueryBalanceResponse,
  type QueryOrderResponse,
  type UnifiedOrderResponse
} from "../lib/paymentApi6";
import { payment6ApiDocs, payment6InterfaceRules, payment6OrderStates, payment6SignRules } from "../lib/paymentApi6Docs";

type PaymentTest6WorkspaceProps = {
...
export function PaymentTest6Workspace({ showSuccess, showError }: PaymentTest6WorkspaceProps) {
  const [config, setConfig] = useState<Payment6Config>(DEFAULT_PAYMENT6_CONFIG);
  const [useProxyApi, setUseProxyApi] = useState(() =>
    isProxyPayment6ApiConfig({
      unifiedOrderApi: DEFAULT_PAYMENT6_CONFIG.unifiedOrderApi,
      queryOrderApi: DEFAULT_PAYMENT6_CONFIG.queryOrderApi,
      queryBalanceApi: DEFAULT_PAYMENT6_CONFIG.queryBalanceApi
    })
  );
  ...
  const [orderResult, setOrderResult] = useState<Payment6ApiResult<UnifiedOrderResponse> | null>(null);
  const [queryResult, setQueryResult] = useState<Payment6ApiResult<QueryOrderResponse> | null>(null);
  const [balanceResult, setBalanceResult] = useState<Payment6ApiResult<QueryBalanceResponse> | null>(null);
  const [callbacks, setCallbacks] = useState<Payment6Callback[]>([]);
  const [selectedApiDoc, setSelectedApiDoc] = useState<ApiDocInterface>(payment6ApiDocs[0]);
  const [logs, setLogs] = useState<LogEntry[]>([]);
  const [logIdCounter, setLogIdCounter] = useState(0);

  useEffect(() => {
    const urls = getDefaultPayment6CallbackUrls();
    setConfig((prev) => ({
      ...prev,
  ...
  async function refreshCallbacks() {
    ...
    try {
      const response = await fetchPayment6Callbacks(50);
      setCallbacks(response.data ?? []);
    } catch (error) {
      console.error("Failed to fetch payment6 callbacks:", error);
    } finally {
  ...
  async function handleClearCallbacks() {
    try {
      await clearPayment6Callbacks();
      setCallbacks([]);
      showSuccess("拉单测试6回调记录已清空");
    } catch (error) {
      showError(error);
    }
  }

  function updateConfig(key: keyof Payment6Config, value: string) {
    setConfig((prev) => ({
      ...prev,
      [key]: value
    }));
  ...
  return (
    <div className="space-y-6">
      <Card className="soft-panel rounded-[32px] border-0">
        <CardHeader>
          <div>
            <CardTitle className="display-font text-2xl font-bold">拉单测试6</CardTitle>
            <CardDescription className="mt-1 text-sm text-stone-600">
              万盛通道的可视化测试页，支持统一下单、订单查询、余额查询、回调记录和静态文档查看。
            </CardDescription>
          </div>
        </CardHeader>
      </Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between rounded-2xl border border-stone-200 bg-white/80 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-stone-500">开启后经由 /api/payment6-proxy 转发到万盛支付域名。</p>
            </div>
          ...
          <div className="rounded-[24px] border border-dashed border-stone-300 bg-white/70 p-4 text-sm leading-7 text-stone-600">
            <p><strong>商户名称：</strong><span className="ml-2">金安</span></p>
            <p><strong>商户后台：</strong><span className="ml-2 break-all">https://jkmch-wansheng.douyaya.com</span></p>
            <p><strong>登录账号：</strong><span className="ml-2">jinan</span></p>
              ...
              <div className="space-y-4 rounded-[24px] border border-stone-200 bg-white/80 p-4">
                ...
                {queryResult.data ? (
                  <div className="grid gap-2 text-sm text-stone-700">
                    ...
                    <p>订单状态：{PAYMENT6_STATE_MAP[queryResult.data.state] || queryResult.data.state}</p>
                  ...
                  <div key={callback.id} className="space-y-4 rounded-[24px] border border-stone-200 bg-white/80 p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Chip className={`px-3 py-1 text-xs font-semibold ${isPaid ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-amber-200 bg-amber-50 text-amber-700"}`}>
                        {PAYMENT6_STATE_MAP[callback.state] || `状态 ${callback.state}`}
                      </Chip>
      ...
      <Card className="soft-panel rounded-[32px] border-0">
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 lg:grid-cols-4">
            {payment6InterfaceRules.map((rule) => (
          ...
          <ApiDocSelector apis={payment6ApiDocs} selectedApi={selectedApiDoc} onSelect={setSelectedApiDoc} />
