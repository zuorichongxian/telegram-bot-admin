...
import { DEFAULT_PAYMENT5_MERCHANT_KEY } from "../utils/payment5Sign.js";

function rewritePayment5Sign(req: Request) {
  ...
  body.sign = generateSign(signParams, DEFAULT_PAYMENT5_MERCHANT_KEY);
  body.key = DEFAULT_PAYMENT5_MERCHANT_KEY;
}
...
export const payment5Proxy = createProxyMiddleware({
  ...
  on: {
    proxyReq: (proxyReq, req) => {
      rewritePayment5Sign(req as Request);
    ...
    proxyRes: (proxyRes, req) => {
      ...
      proxyRes.on("end", () => {
        ...
        console.log("[payment5-proxy] response", {
          method: req.method,
          path: req.url,
    ...
    error: (err, req, res) => {
      console.error("[payment5-proxy] error", {
        method: req.method,
        path: req.url,
        message: err.message
      });
