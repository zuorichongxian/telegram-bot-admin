...
import { DEFAULT_PAYMENT6_MERCHANT_KEY } from "../utils/payment6Sign.js";

function rewritePayment6Sign(req: Request) {
  ...
  body.sign = generateSign(signParams, DEFAULT_PAYMENT6_MERCHANT_KEY);
  body.key = DEFAULT_PAYMENT6_MERCHANT_KEY;
}

export const payment6Proxy = createProxyMiddleware({
  target: "https://jkapi-wansheng.douyaya.com",
  changeOrigin: true,
  proxyTimeout: 15000,
  timeout: 20000,
  ...
  on: {
    proxyReq: (proxyReq, req) => {
      rewritePayment6Sign(req as Request);
    ...
    proxyRes: (proxyRes, req) => {
      ...
      proxyRes.on("end", () => {
        ...
        console.log("[payment6-proxy] response", {
          method: req.method,
          path: req.url,
    ...
    error: (err, req, res) => {
      console.error("[payment6-proxy] error", {
        method: req.method,
        path: req.url,
        message: err.message
      });
