...
import {
  ...
  createUnifiedOrder,
  DEFAULT_PAYMENT8_CONFIG,
  fetchPayment8Callbacks,
  formatFenToYuan,
  generateOrderNo,
  getDefaultPayment8CallbackUrls,
  getPayment8ApiConfig,
...
} from "../lib/paymentApi8";
import { payment8ApiDocs, payment8InterfaceRules, payment8OrderStates, payment8SignRules } from "../lib/paymentApi8Docs";

type PaymentTest8WorkspaceProps = {
...
export function PaymentTest8Workspace({ showSuccess, showError }: PaymentTest8WorkspaceProps) {
  const [config, setConfig] = useState<Payment8Config>(DEFAULT_PAYMENT8_CONFIG);
  const [useProxyApi, setUseProxyApi] = useState(() =>
    isProxyPayment8ApiConfig({
      unifiedOrderApi: DEFAULT_PAYMENT8_CONFIG.unifiedOrderApi,
      queryOrderApi: DEFAULT_PAYMENT8_CONFIG.queryOrderApi,
      queryBalanceApi: DEFAULT_PAYMENT8_CONFIG.queryBalanceApi
    })
  );
  ...
  const [orderResult, setOrderResult] = useState<Payment8ApiResult<UnifiedOrderResponse> | null>(null);
  const [queryResult, setQueryResult] = useState<Payment8ApiResult<QueryOrderResponse> | null>(null);
  const [balanceResult, setBalanceResult] = useState<Payment8ApiResult<QueryBalanceResponse> | null>(null);
  const [callbacks, setCallbacks] = useState<Payment8Callback[]>([]);
  const [selectedApiDoc, setSelectedApiDoc] = useState<ApiDocInterface>(payment8ApiDocs[0]);
  const [logs, setLogs] = useState<LogEntry[]>([]);
  const [logIdCounter, setLogIdCounter] = useState(0);

  useEffect(() => {
    const urls = getDefaultPayment8CallbackUrls();
    setConfig((prev) => ({
      ...prev,
  ...
  async function refreshCallbacks() {
    ...
    try {
      const response = await fetchPayment8Callbacks(50);
      setCallbacks(response.data ?? []);
    } catch (error) {
      console.error("Failed to fetch payment8 callbacks:", error);
    } finally {
  ...
  async function handleClearCallbacks() {
    try {
      await clearPayment8Callbacks();
      setCallbacks([]);
      showSuccess("接单测试8回调记录已清空");
    } catch (error) {
      showError(error);
    }
  }

  function updateConfig(key: keyof Payment8Config, value: string) {
    setConfig((prev) => ({
      ...prev,
      [key]: value
    }));
  ...
  return (
    <div className="space-y-6">
      <Card className="soft-panel rounded-[32px] border-0">
        <CardHeader>
          <div>
            <CardTitle className="display-font text-2xl font-bold">接单测试8</CardTitle>
            <CardDescription className="mt-1 text-sm text-stone-600">
              晚州通道的可视化测试页，支持统一下单、订单查询、余额查询、回调记录和静态文档查看。
            </CardDescription>
          </div>
        </CardHeader>
      </Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between rounded-2xl border border-stone-200 bg-white/80 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-stone-500">开启后经由 /api/payment8-proxy 转发到晚州支付域名。</p>
            </div>
          ...
          <div className="rounded-[24px] border border-dashed border-stone-300 bg-white/70 p-4 text-sm leading-7 text-stone-600">
            <p><strong>商户名称：</strong><span className="ml-2">晚州</span></p>
            <p><strong>商户后台：</strong><span className="ml-2 break-all">-</span></p>
            <p><strong>登录账号：</strong><span className="ml-2">晚州</span></p>
            <p><strong>商务号：</strong><span className="ml-2">260452181</span></p>
              ...
              <div className="space-y-4 rounded-[24px] border border-stone-200 bg-white/80 p-4">
                ...
                {queryResult.data ? (
                  <div className="grid gap-2 text-sm text-stone-700">
                    ...
                    <p>订单状态：{PAYMENT8_STATE_MAP[queryResult.data.state] || queryResult.data.state}</p>
                  ...
                  <div key={callback.id} className="space-y-4 rounded-[24px] border border-stone-200 bg-white/80 p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Chip className={`px-3 py-1 text-xs font-semibold ${isPaid ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-amber-200 bg-amber-50 text-amber-700"}`}>
                        {PAYMENT8_STATE_MAP[callback.state] || `状态 ${callback.state}`}
                      </Chip>
      ...
      <Card className="soft-panel rounded-[32px] border-0">
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 lg:grid-cols-4">
            {payment8InterfaceRules.map((rule) => (
          ...
          <ApiDocSelector apis={payment8ApiDocs} selectedApi={selectedApiDoc} onSelect={setSelectedApiDoc} />
