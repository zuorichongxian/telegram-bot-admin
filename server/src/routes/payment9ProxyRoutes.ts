...
import {
  generatePayment9Sign,
  DEFAULT_PAYMENT9_CLIENT_SECRET
} from "../utils/payment9Sign.js";

function rewritePayment9Sign(req: Request) {
  ...
  body.sign = generatePayment9Sign(signParams, DEFAULT_PAYMENT9_CLIENT_SECRET);
}

export const payment9Proxy = createProxyMiddleware({
  ...
  on: {
    proxyReq: (proxyReq, req) => {
      rewritePayment9Sign(req as Request);
    ...
    proxyRes: (proxyRes, req) => {
      ...
      proxyRes.on("end", () => {
        ...
        console.log("[payment9-proxy] response", {
          method: req.method,
          path: req.url,
    ...
    error: (err, req, res) => {
      console.error("[payment9-proxy] error", {
        method: req.method,
        path: req.url,
        message: err.message
      });
