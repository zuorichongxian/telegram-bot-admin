...
import { DEFAULT_PAYMENT4_MERCHANT_KEY, generatePayment4Sign } from "../utils/payment4Sign.js";

function rewritePayment4Sign(req: Request) {
  const body = req.body as Record<string, unknown> | undefined;
  if (!body || typeof body !== "object") {
    return;
  }

  const signParams: Record<string, string | number | undefined | null> = {};
  ...
  body.sign = generatePayment4Sign(signParams, DEFAULT_PAYMENT4_MERCHANT_KEY);
}

export const payment4Proxy = createProxyMiddleware({
  target: "http://xiaohuob.aspay.one",
  changeOrigin: true,
  proxyTimeout: 15000,
  timeout: 20000,
  ...
  on: {
    proxyReq: (proxyReq, req) => {
      rewritePayment4Sign(req as Request);
    ...
    proxyRes: (proxyRes, req) => {
      ...
      proxyRes.on("end", () => {
        ...
        console.log("[payment4-proxy] response", {
          method: req.method,
          path: req.url,
    ...
    error: (err, req, res) => {
      console.error("[payment4-proxy] error", {
        method: req.method,
        path: req.url,
        message: err.message
      });
