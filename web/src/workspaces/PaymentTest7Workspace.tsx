...
import {
  clearPayment7Callbacks,
  createUnifiedOrder,
  DEFAULT_PAYMENT7_CONFIG,
  fetchPayment7Callbacks,
  formatFenToYuan,
  generateOrderNo,
  getDefaultPayment7CallbackUrls,
  getPayment7ApiConfig,
  isProxyPayment7ApiConfig,
  isValidFenAmount,
  maskKey,
  PAYMENT7_STATE_MAP,
  queryBalance,
  queryOrder,
  type Payment7ApiResult,
  type Payment7Callback,
  type Payment7Config,
  type QueryBalanceResponse,
  type QueryOrderResponse,
  type UnifiedOrderResponse
} from "../lib/paymentApi7";
import { payment7ApiDocs, payment7InterfaceRules, payment7OrderStates, payment7SignRules } from "../lib/paymentApi7Docs";

type PaymentTest7WorkspaceProps = {
...
export function PaymentTest7Workspace({ showSuccess, showError }: PaymentTest7WorkspaceProps) {
  const [config, setConfig] = useState<Payment7Config>(DEFAULT_PAYMENT7_CONFIG);
  const [useProxyApi, setUseProxyApi] = useState(() =>
    isProxyPayment7ApiConfig({
      unifiedOrderApi: DEFAULT_PAYMENT7_CONFIG.unifiedOrderApi,
      queryOrderApi: DEFAULT_PAYMENT7_CONFIG.queryOrderApi,
      queryBalanceApi: DEFAULT_PAYMENT7_CONFIG.queryBalanceApi
    })
  );
  ...
  const [orderResult, setOrderResult] = useState<Payment7ApiResult<UnifiedOrderResponse> | null>(null);
  const [queryResult, setQueryResult] = useState<Payment7ApiResult<QueryOrderResponse> | null>(null);
  const [balanceResult, setBalanceResult] = useState<Payment7ApiResult<QueryBalanceResponse> | null>(null);
  const [callbacks, setCallbacks] = useState<Payment7Callback[]>([]);
  const [selectedApiDoc, setSelectedApiDoc] = useState<ApiDocInterface>(payment7ApiDocs[0]);
  const [logs, setLogs] = useState<LogEntry[]>([]);
  const [logIdCounter, setLogIdCounter] = useState(0);

  useEffect(() => {
    const urls = getDefaultPayment7CallbackUrls();
    setConfig((prev) => ({
      ...prev,
  ...
  async function refreshCallbacks() {
    ...
    try {
      const response = await fetchPayment7Callbacks(50);
      setCallbacks(response.data ?? []);
    } catch (error) {
      console.error("Failed to fetch payment7 callbacks:", error);
    } finally {
  ...
  async function handleClearCallbacks() {
    try {
      await clearPayment7Callbacks();
      setCallbacks([]);
      showSuccess("接单测试7回调记录已清空");
    } catch (error) {
      showError(error);
    }
  }

  function updateConfig(key: keyof Payment7Config, value: string) {
    setConfig((prev) => ({
      ...prev,
      [key]: value
    }));
  ...
  return (
    <div className="space-y-6">
      <Card className="soft-panel rounded-[32px] border-0">
        <CardHeader>
          <div>
            <CardTitle className="display-font text-2xl font-bold">接单测试7 - 金安支付通道</CardTitle>
            <CardDescription className="mt-1 text-sm text-stone-600">
              金安支付通道的可视化测试页，支持统一下单、订单查询、余额查询、回调记录和静态文档查看。
            </CardDescription>
          </div>
        </CardHeader>
      </Card>
      ...
      <Card className="soft-panel rounded-[32px] border-0">
        <CardHeader>
          ...
          <CardDescription className="text-sm text-stone-600">金额单位统一为元</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between rounded-2xl border border-stone-200 bg-white/80 px-4 py-3">
            <div>
              ...
              <p className="text-xs text-stone-500">开启后经由 /api/payment7-proxy 转发到金安支付域名。</p>
            </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-wrap gap-2">
                      {QUICK_AMOUNTS.map((amount) => (
                        <Chip
                          key={amount}
                          className={`cursor-pointer px-3 py-1.5 text-sm font-medium transition ${orderForm.amount === String(amount) ? "border border-blue-400 bg-blue-50 text-blue-700" : "border border-stone-300 bg-white text-stone-600 hover:border-blue-300 hover:text-blue-600"}`}
                          onClick={() => updateOrderForm("amount", String(amount))}
                        >
                          {amount} 元
                        </Chip>
                      ))}
              ...
              <div className="space-y-4 rounded-[24px] border border-stone-200 bg-white/80 p-4">
                ...
                {queryResult.data ? (
                  <div className="grid gap-2 text-sm text-stone-700">
                    ...
                    <p>订单状态：{PAYMENT7_STATE_MAP[queryResult.data.state] || queryResult.data.state}</p>
                  ...
                  <div key={callback.id} className="space-y-4 rounded-[24px] border border-stone-200 bg-white/80 p-4">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Chip className={`px-3 py-1 text-xs font-semibold ${isPaid ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-amber-200 bg-amber-50 text-amber-700"}`}>
                        {PAYMENT7_STATE_MAP[callback.state] || `状态 ${callback.state}`}
                      </Chip>
      ...
      <Card className="soft-panel rounded-[32px] border-0">
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 lg:grid-cols-4">
            {payment7InterfaceRules.map((rule) => (
          ...
          <ApiDocSelector apis={payment7ApiDocs} selectedApi={selectedApiDoc} onSelect={setSelectedApiDoc} />
