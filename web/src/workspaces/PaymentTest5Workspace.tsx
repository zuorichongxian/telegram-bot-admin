...
export function PaymentTest5Workspace({ showSuccess, showError }: PaymentTest5WorkspaceProps) {
  const [config, setConfig] = useState<Payment5Config>(DEFAULT_PAYMENT5_CONFIG);
  const [useProxyApi, setUseProxyApi] = useState(() =>
    isProxyPayment5ApiConfig({
      unifiedOrderApi: DEFAULT_PAYMENT5_CONFIG.unifiedOrderApi,
      queryOrderApi: DEFAULT_PAYMENT5_CONFIG.queryOrderApi,
      queryBalanceApi: DEFAULT_PAYMENT5_CONFIG.queryBalanceApi
    })
  );
  ...
  const [orderResult, setOrderResult] = useState<Payment5ApiResult<UnifiedOrderResponse> | null>(null);
  const [queryResult, setQueryResult] = useState<Payment5ApiResult<QueryOrderResponse> | null>(null);
  const [balanceResult, setBalanceResult] = useState<Payment5ApiResult<QueryBalanceResponse> | null>(null);
  const [callbacks, setCallbacks] = useState<Payment5Callback[]>([]);
  const [selectedApiDoc, setSelectedApiDoc] = useState<ApiDocInterface>(payment5ApiDocs[0]);
  const [logs, setLogs] = useState<LogEntry[]>([]);
  const [logIdCounter, setLogIdCounter] = useState(0);

  useEffect(() => {
    const urls = getDefaultPayment5CallbackUrls();
    setConfig((prev) => ({
      ...prev,
  ...
  async function refreshCallbacks() {
    ...
    try {
      const response = await fetchPayment5Callbacks(50);
      setCallbacks(response.data ?? []);
    } catch (error) {
      console.error("Failed to fetch payment5 callbacks:", error);
    } finally {
  ...
  async function handleClearCallbacks() {
    try {
      await clearPayment5Callbacks();
      setCallbacks([]);
      showSuccess("拉单测试5回调记录已清空");
    } catch (error) {
      showError(error);
    }
  }

  function updateConfig(key: keyof Payment5Config, value: string) {
    setConfig((prev) => ({
      ...prev,
      [key]: value
    }));
      ...
      <Card className="soft-panel rounded-[32px] border-0">
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 lg:grid-cols-4">
            {payment5InterfaceRules.map((rule) => (
          ...
          <ApiDocSelector apis={payment5ApiDocs} selectedApi={selectedApiDoc} onSelect={setSelectedApiDoc} />
